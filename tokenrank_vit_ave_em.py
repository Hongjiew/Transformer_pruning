...
class Attention(nn.Module):
    ...
    def forward(self, x, token_mask, eps = 1e-6):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)

        attn = (q @ k.transpose(-2, -1)) * self.scale  #attn size [B, H, N, N]
    ...
    def get_importance(self, attention_score, token_mask, tau_imp=0.1, eps = 1e-6, iters=5, d=0.):
        ...
        #attn_mean = (attention_score/tau_imp).pow(2)
        #attn_mean = torch.mean(attn_mean,dim=1).pow(0.5)
        attn_mean = attention_score/tau_imp
        if token_mask is not None:
            attn = attn_mean.exp()*token_mask[:,None,None,:]
        else:
            attn = attn_mean.exp()
        M = attn/ attn.sum(dim=-1, keepdim=True) 
        #M = torch.mean(M.pow(2),dim=1).pow(0.5)
        #M , _ = torch.max(M,dim=1)
        #M = torch.mean(M,dim=1)
        #dist  = (torch.ones(B,H,1,N)/N).to(M.device)
        #for i in range(iters):
        #    dist = (dist@M)*(1-d) + d/N
        dist = torch.mean(M,dim=2)
        dist = torch.mean(dist.pow(2),dim=1).pow(0.5)
        #dist = torch.mean(M,dim=1)
...
class TokenRankVisionTransformer(nn.Module):
    ...
    def __init__(self, prune_list, retain_rate_list, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, tau_imp = 0.5, weight_init=''):
        """
        Args:
        ...
        self.blocks =  nn.ModuleList([
            Block(
                ...
                tau_imp = tau_imp, retain_rate = retain_rate_list[prune_list.index(i)]) if i in prune_list else 

            Block(
                prune = False, dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, 
                tau_imp = tau_imp,retain_rate = 1)
